...
<div id="mainContent">
    ...
    <div class="posters">
      ...
      <div class="poster-card">
        ...
        <div class="poster-body">
          ...
          <button class="poster-cta" data-gender="laki">Tempah Baju Banin</button>
        </div>
      </div>
      ...
      <div class="poster-card">
        ...
        <div class="poster-body">
          ...
          <button class="poster-cta" data-gender="muslimah">Tempah Baju Banat</button>
        </div>
      </div>
  ...
  <section id="saiz" class="reveal">
    ...
    <div class="size-gender-tabs">
      <button class="sgt-btn active" data-gender="laki">Baju Laki</button>
      <button class="sgt-btn" data-gender="muslimah">Baju Muslimah</button>
    </div>
      ...
      <div class="card">
        ...
        <div class="qty-block" id="blockLaki" style="flex-direction:column;align-items:stretch;">
          <div style="display:flex;align-items:center;justify-content:space-between;">
            <div class="lbl">Baju Laki<span id="rateLakiLabel">Pilih combo dahulu</span></div>
        ...
        <div class="qty-block" id="blockMuslimah" style="flex-direction:column;align-items:stretch;">
          <div style="display:flex;align-items:center;justify-content:space-between;">
            <div class="lbl">Baju Muslimah<span id="rateMuslimahLabel">Pilih combo dahulu</span></div>
            <div class="stepper" style="justify-content:flex-end;">
      ...
      <table class="admin-table">
        <thead>
          <tr>
            ...
            <th>Combo</th><th>Banin</th><th>Banat</th><th>Jumlah</th><th>Komisen</th><th>Catatan</th><th>Resit</th>
          </tr>
        </thead>
      ...
      <div>
        ...
        <div class="tblwrap">
          <table class="admin-table" id="sizeSummaryLakiTable">
            <thead><tr><th>Saiz</th><th>Jumlah helai</th></tr></thead>
            <tbody></tbody>
          </table>
        </div>
      </div>
...
function computeOrder(){
  ...
  return {qL,qM,tier,rL:rates.laki,rM:rates.muslimah,subL,subM,total:grandTotal,commission,qtySum:sum};
}
...
function renderReceipt(){
  ...
  let html = '';
  if(o.qL > 0){
    html += `<div class="rline"><span>Baju Laki x${o.qL}<div class="sub">@ ${fmt(o.rL)} / pc</div></span><span>${fmt(o.subL)}</span></div>`;
  }
  if(o.qM > 0){
    html += `<div class="rline"><span>Baju Muslimah x${o.qM}<div class="sub">@ ${fmt(o.rM)} / pc</div></span><span>${fmt(o.subM)}</span></div>`;
  }
...
async function loadOrders(){
  ...
  tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;color:var(--ink-soft);">Memuatkan...</td></tr>';
  try{
    ...
    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;color:var(--danger);">Gagal muatkan pesanan.</td></tr>';
  }
}
...
function renderSizeSummary(orders){
  ...
  SIZES.forEach(sz=>{ lakiTotals[sz] = 0; muslimahTotals[sz] = 0; });
  orders.forEach(o=>{
    if(o.sizeLaki){
  ...
  lakiBody.innerHTML = SIZES.map(sz=>`<tr><td>${sz}</td><td>${lakiTotals[sz]}</td></tr>`).join('')
    + `<tr style="font-weight:600;"><td>Jumlah</td><td>${lakiGrand}</td></tr>`;

  const muslimahBody = document.querySelector('#sizeSummaryMuslimahTable tbody');
  muslimahBody.innerHTML = SIZES.map(sz=>`<tr><td>${sz}</td><td>${muslimahTotals[sz]}</td></tr>`).join('')
    + `<tr style="font-weight:600;"><td>Jumlah</td><td>${muslimahGrand}</td></tr>`;
}

function renderOrders(orders){
  const tbody = document.getElementById('ordersTbody');
  if(orders.length === 0){
    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;color:var(--ink-soft);">Belum ada pesanan.</td></tr>';
    return;
  }
  tbody.innerHTML = orders.map((o)=>{
    ...
    const receiptCell = o.receiptUrl
      ...
      : '-';
    const comboLabel = o.comboType === 'fixed' ? `${o.comboTarget} helai` : (o.comboType === 'custom' ? 'Sendiri' : '-');
    return `<tr>
      <td>${dateStr}</td>
