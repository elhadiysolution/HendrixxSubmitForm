...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .bank-info{background:var(--teal-light);border:1px solid var(--teal);border-radius:10px;padding:14px 16px;margin-bottom:14px;}
  .bank-info-head{font-family:'Space Grotesk',sans-serif;font-weight:700;font-size:13px;color:var(--navy);letter-spacing:.02em;margin-bottom:10px;}
  .bank-info-row{display:flex;align-items:center;justify-content:space-between;gap:8px;font-size:13px;padding:5px 0;}
  .bank-info-row + .bank-info-row{border-top:1px dashed rgba(21,79,72,0.15);}
  .bank-info-row .bi-label{color:var(--ink-soft);font-size:12px;}
  .bank-info-row .bi-value{display:flex;align-items:center;gap:8px;font-family:'IBM Plex Mono',monospace;font-weight:600;color:var(--navy);}
  .bi-copy{border:1px solid var(--line);background:#fff;border-radius:6px;padding:3px 9px;font-size:11px;font-weight:600;color:var(--navy);cursor:pointer;font-family:'Inter',sans-serif;}
  .bi-copy:hover{border-color:var(--navy);}
  ...
  .bulk-accordion{border:1px solid var(--line);border-radius:14px;background:var(--panel);overflow:hidden;}
  .bulk-accordion-summary{
    cursor:pointer;list-style:none;padding:18px 22px;font-family:'Space Grotesk',sans-serif;font-weight:700;
    font-size:16px;color:var(--navy);display:flex;align-items:center;gap:10px;
  }
  .bulk-accordion-summary::-webkit-details-marker{display:none;}
  .bulk-accordion-summary::before{content:'▸';font-size:13px;color:var(--ink-soft);transition:transform .15s ease;}
  .bulk-accordion[open] .bulk-accordion-summary::before{transform:rotate(90deg);}
  .bulk-accordion-summary:hover{background:#F4F7F5;}
  .bulk-accordion-body{padding:4px 22px 24px;border-top:1px solid var(--line);}
  .bk-order-block{position:relative;max-width:720px;margin:20px 0;}
  .bk-remove-btn{
    position:absolute;top:14px;right:14px;background:none;border:1px solid var(--line);border-radius:7px;
    padding:5px 11px;font-size:12px;font-weight:600;color:var(--danger);cursor:pointer;font-family:'Inter',sans-serif;
  }
  .bulk-actions{display:flex;gap:10px;margin-top:6px;flex-wrap:wrap;}
  .bulk-actions .submit-btn{width:auto;margin-top:0;padding:11px 22px;}
  ...
  .pagination button{
    min-width:32px;height:32px;padding:0 8px;border-radius:7px;border:1px solid var(--line);background:#fff;
    font-size:12.5px;font-weight:600;color:var(--navy);cursor:pointer;font-family:'Inter',sans-serif;
  }
  ...
  .pagination .pg-info{font-size:12px;color:var(--ink-soft);margin:0 6px;}
  ...
  .delivery-cards{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:8px;}
  @media (max-width:600px){.delivery-cards{grid-template-columns:1fr;}}
  .delivery-card{
    border:1.5px solid var(--line);border-radius:12px;padding:14px 12px;text-align:center;
    cursor:pointer;background:#fff;transition:border-color .15s ease,background .15s ease,transform .15s ease;
  }
  .delivery-card:hover{border-color:var(--navy);transform:translateY(-2px);}
  .delivery-card.selected{border-color:var(--navy);background:var(--teal-light);}
  .delivery-card .dc-title{font-family:'Space Grotesk',sans-serif;font-weight:700;font-size:14px;color:var(--navy);}
  .delivery-card .dc-sub{font-size:11.5px;color:var(--ink-soft);margin-top:3px;}
...
<div id="mainContent">
  ...
  <section id="mainposter" class="reveal" style="padding-bottom:0;">
    ...
    <div class="main-poster-wrap">
      <img src="{{ asset('images/earlybird.jpeg') }}" alt="Poster early bird 5th Edition Emerald Chapter" class="main-poster-img">
    </div>
  </section>
  ...
  <section id="order" class="reveal">
    ...
    <div class="grid">
      <div class="card">
        ...
        <h3 class="formh" style="margin-top:22px;"><span class="num">4</span>Kaedah Penghantaran <span class="required-tag">*</span></h3>
        ...
        <h3 class="formh"><span class="num">5</span>Bukti Pembayaran <span class="required-tag">*</span></h3>
        <div class="bank-info">
          <div class="bank-info-head">MAYBANK RASMI HDRX EXCLUSIVE</div>
          <div class="bank-info-row">
            <span class="bi-label">Bank</span>
            <span class="bi-value">Maybank</span>
          </div>
          <div class="bank-info-row">
          ...
          <div class="bank-info-row">
            ...
            <span class="bi-value">HENDRIXX EXCLUSIVE EMPIRE</span>
          </div>
        </div>
        <div class="field">
...
<div id="adminSection">
  <section style="padding-top:40px;">
    ...
    <details class="bulk-accordion" style="margin-top:44px;">
      <summary class="bulk-accordion-summary">Bulk Submit Pesanan (Offline) — untuk order yang ejen kumpul secara offline, tanpa resit</summary>
      <div class="bulk-accordion-body">
        <div id="bkOrdersContainer"></div>
        <div class="bulk-actions">
          <button type="button" class="refresh-btn" id="bkAddOrderBtn">+ Tambah Order Lain</button>
          <button type="button" class="submit-btn" id="bkSubmitAllBtn">Hantar Semua Pesanan</button>
        </div>
        <div class="err" id="bkGlobalErr" style="margin-top:10px;"></div>
      </div>
    </details>
  </section>
</div>
...
function buildSizeGrid(containerId, prefix, onChange){
  onChange = onChange || updateSplitAndReceipt;
...
const LYC_PREFIX_BY_SIZE_PREFIX = { szL:'lycL', szM:'lycM' };
...
function clampAllLycraFor(szPrefixL, szPrefixM){
  SIZES.forEach(sz=>{
    clampLycraForSize(szPrefixL, sz);
    clampLycraForSize(szPrefixM, sz);
  });
}
function clampAllLycra(){ clampAllLycraFor('szL', 'szM'); }
function buildLycraGrid(containerId, lycraPrefix, sizePrefix, onChange){
  onChange = onChange || updateSplitAndReceipt;
  const container = document.getElementById(containerId);
  container.innerHTML = SIZES.map(sz => `
    <div class="size-input">
...
function sizeSurchargeBreakdown(prefix){
  const counts = getSizeCounts(prefix);
  let band4 = 0, band6 = 0;
  SIZES.forEach(sz=>{
    const rate = sizeSurchargeFor(sz);
    if(rate === 4) band4 += counts[sz];
    else if(rate === 6) band6 += counts[sz];
  });
...
function buildReceiptHtml(o){
  ...
  const surchargeLines = (label, sur)=>{
    let out = '';
    if(sur.band4 > 0){
      out += `<div class="rline"><span>${label} — saiz 3XL–5XL x${sur.band4}<div class="sub">@ +${fmt(4)} / pc</div></span><span>${fmt(sur.band4 * 4)}</span></div>`;
    }
    if(sur.band6 > 0){
      out += `<div class="rline"><span>${label} — saiz 6XL–8XL x${sur.band6}<div class="sub">@ +${fmt(6)} / pc</div></span><span>${fmt(sur.band6 * 6)}</span></div>`;
    }
    return out;
  };
  let html = '';
  if(o.qL > 0){
    ...
    html += surchargeLines('Baju Laki', o.surL);
  }
  if(o.qM > 0){
...
let bkAgentOptionsHtml = '<option value="">Pilih ejen</option>';
async function loadAgentOptions(){
  ...
  try{
    ...
    bkAgentOptionsHtml = optionsHtml;
    document.querySelectorAll('.bk-agent-select').forEach(sel=>{ sel.innerHTML = optionsHtml; });
  }catch(e){
    select.innerHTML = '<option value="">Gagal muat senarai ejen</option>';
    bkAgentOptionsHtml = '<option value="">Gagal muat senarai ejen</option>';
    document.querySelectorAll('.bk-agent-select').forEach(sel=>{ sel.innerHTML = bkAgentOptionsHtml; });
  }
}
loadAgentOptions();
...
submitBtn.addEventListener('click', async ()=>{
  ...
  if(!deliveryMethod){
    formErr.textContent = 'Sila pilih kaedah penghantaran (sekolah atau rumah).';
    formErr.style.display = 'block';
    document.getElementById('deliveryCards').scrollIntoView({behavior:'smooth', block:'center'});
    return;
  }
  if(deliveryMethod === 'home' && !custAddr){
    formErr.textContent = 'Sila isi alamat penghantaran untuk hantar ke rumah.';
    formErr.style.display = 'block';
    document.getElementById('custAddr').scrollIntoView({behavior:'smooth', block:'center'});
    return;
  }
  if(!receiptBase64){
...
/* admin bulk (offline) order entry — supports multiple queued orders at once */
function bkComboCardsHtml(){
  const earlybirdActive = new Date() <= new Date('2026-09-05T15:59:59Z');
...
function bkOrderBlockHtml(idx){
  const p = 'bk' + idx;
  return `
    <div class="card bk-order-block" id="${p}Block">
      <button type="button" class="bk-remove-btn" data-remove="${idx}">✕ Buang</button>
      <h3 class="formh" style="margin-top:0;"><span class="num">${idx+1}</span>Order #${idx+1}</h3>
      <div class="field">
        <label>Nama ejen *</label>
        <select id="${p}AgentName" class="bk-agent-select">${bkAgentOptionsHtml}</select>
      </div>
      <div class="field">
        <label>Sekolah *</label>
        <select id="${p}SchoolName">
          <option value="">Pilih sekolah</option>
          <option value="Mits Istana Bandar">Mits Istana Bandar</option>
          <option value="Mits Alam Impian">Mits Alam Impian</option>
          <option value="Mits Pandan Indah">Mits Pandan Indah</option>
      ...
      <div class="row2">
        <div class="field">
          <label>Nama pelanggan <span class="required-tag">*</span></label>
          <input type="text" id="${p}CustName" placeholder="Nama pelanggan">
        </div>
        <div class="field">
          <label>No. telefon</label>
          <input type="tel" id="${p}CustPhone" placeholder="01X-XXXXXXX">
        </div>
      </div>
      <div class="field">
        <label>Pilih Combo</label>
        <div class="combo-cards" id="${p}ComboCards">${bkComboCardsHtml()}</div>
        <div class="combo-hint" id="${p}ComboHint">Sila pilih combo di atas dahulu sebelum tetapkan kuantiti.</div>
      </div>
      <div class="qty-block" id="${p}BlockLaki" style="flex-direction:column;align-items:stretch;">
        <div style="display:flex;align-items:center;justify-content:space-between;">
          <div class="lbl">Baju Banin<span id="${p}RateLakiLabel">Pilih combo dahulu</span></div>
          <div class="bk-stepper" style="justify-content:flex-end;">
            <button type="button" data-target="${p}QtyLaki" data-step="-1" disabled>−</button>
            <input type="number" id="${p}QtyLaki" value="0" min="0" disabled>
            <button type="button" data-target="${p}QtyLaki" data-step="1" disabled>+</button>
          </div>
        </div>
        <div class="size-grid" id="${p}SizeGridLaki"></div>
        <div class="size-status" id="${p}SizeStatusLaki"></div>
        <div class="upgrade-row" style="flex-direction:column;align-items:stretch;">
          <div class="lbl">Naik taraf Lycra Premium (ikut saiz)<span>+RM10 / helai</span></div>
          <div class="size-grid" id="${p}LycraGridLaki"></div>
          <div class="size-status" id="${p}LycraStatusLaki"></div>
        </div>
      </div>
      <div class="qty-block" id="${p}BlockMuslimah" style="flex-direction:column;align-items:stretch;">
        <div style="display:flex;align-items:center;justify-content:space-between;">
          <div class="lbl">Baju Banat<span id="${p}RateMuslimahLabel">Pilih combo dahulu</span></div>
          <div class="bk-stepper" style="justify-content:flex-end;">
            <button type="button" data-target="${p}QtyMuslimah" data-step="-1" disabled>−</button>
            <input type="number" id="${p}QtyMuslimah" value="0" min="0" disabled>
            <button type="button" data-target="${p}QtyMuslimah" data-step="1" disabled>+</button>
          </div>
        </div>
        <div class="size-grid" id="${p}SizeGridMuslimah"></div>
        <div class="size-status" id="${p}SizeStatusMuslimah"></div>
        <div class="upgrade-row" style="flex-direction:column;align-items:stretch;">
          <div class="lbl">Naik taraf Lycra Premium (ikut saiz)<span>+RM10 / helai</span></div>
          <div class="size-grid" id="${p}LycraGridMuslimah"></div>
          <div class="size-status" id="${p}LycraStatusMuslimah"></div>
        </div>
      </div>
      <div class="split-status" id="${p}SplitStatus"></div>
      <div class="field" style="margin-top:14px;">
        <label>Kaedah Penghantaran</label>
        <div class="delivery-cards" id="${p}DeliveryCards">
          <div class="delivery-card" data-method="school">
            <div class="dc-title">Hantar ke Sekolah</div>
            <div class="dc-sub">Percuma</div>
          </div>
          <div class="delivery-card" data-method="home">
            <div class="dc-title">Hantar ke Rumah</div>
            <div class="dc-sub">+RM8 (1–4 helai) / +RM15 (5+)</div>
          </div>
        </div>
        <div class="combo-hint" id="${p}DeliveryHint">Sila pilih kaedah penghantaran.</div>
      </div>
      <div class="field" id="${p}AddrField" style="display:none;">
        <label>Alamat penghantaran <span class="required-tag">*</span></label>
        <textarea id="${p}CustAddr" placeholder="Alamat penuh untuk posting"></textarea>
      </div>
      <div class="field">
        <label>Catatan (saiz, warna, dll)</label>
        <textarea id="${p}OrderNotes" placeholder="Contoh: 2x saiz M, 1x saiz L"></textarea>
      </div>
      <div class="receipt" style="position:static;margin-top:6px;">
        <div class="receipt-head"><div class="tag">Ringkasan</div><div class="title">Order #${idx+1}</div></div>
        <div class="receipt-body" id="${p}ReceiptBody"><div class="empty-hint">Pilih combo dan masukkan kuantiti</div></div>
      </div>
      <div class="err" id="${p}FormErr" style="margin-top:10px;"></div>
      <div class="split-status" id="${p}SubmitStatus" style="margin-top:10px;display:none;"></div>
    </div>`;
}

let bkOrderCounter = 0;
const bkBlocks = {};

function bkWireOrderBlock(idx){
  const p = 'bk' + idx;
  const state = { selectedCombo: null, deliveryMethod: null };
  bkBlocks[idx] = state;

  const blockEl = document.getElementById(p + 'Block');
  const qtyLakiEl2 = document.getElementById(p + 'QtyLaki');
  const qtyMuslimahEl2 = document.getElementById(p + 'QtyMuslimah');
  const comboHintEl = document.getElementById(p + 'ComboHint');
  const deliveryHintEl = document.getElementById(p + 'DeliveryHint');
  const addrFieldEl = document.getElementById(p + 'AddrField');
  const splitStatusEl = document.getElementById(p + 'SplitStatus');
  const rateLakiLabelEl = document.getElementById(p + 'RateLakiLabel');
  const rateMuslimahLabelEl = document.getElementById(p + 'RateMuslimahLabel');
  const sizeStatusLakiEl = document.getElementById(p + 'SizeStatusLaki');
  const sizeStatusMuslimahEl = document.getElementById(p + 'SizeStatusMuslimah');
  const lycraStatusLakiEl = document.getElementById(p + 'LycraStatusLaki');
  const lycraStatusMuslimahEl = document.getElementById(p + 'LycraStatusMuslimah');
  const receiptBodyEl = document.getElementById(p + 'ReceiptBody');

  QTY_EL_BY_PREFIX[p + 'szL'] = qtyLakiEl2;
  QTY_EL_BY_PREFIX[p + 'szM'] = qtyMuslimahEl2;
  LYC_PREFIX_BY_SIZE_PREFIX[p + 'szL'] = p + 'lycL';
  LYC_PREFIX_BY_SIZE_PREFIX[p + 'szM'] = p + 'lycM';

  function computeThis(){
    const qL = parseInt(qtyLakiEl2.value,10) || 0;
    const qM = parseInt(qtyMuslimahEl2.value,10) || 0;
    return computeOrderFor(qL, qM, state.selectedCombo, state.deliveryMethod, p+'szL', p+'szM', p+'lycL', p+'lycM');
  }
  function comboRemainingThis(excludeEl){
    const otherEl = excludeEl === qtyLakiEl2 ? qtyMuslimahEl2 : qtyLakiEl2;
    return comboRemainingFor(otherEl, state.selectedCombo);
  }
  function updateThis(){
    clampAllLycraFor(p+'szL', p+'szM');
    const qL = parseInt(qtyLakiEl2.value,10) || 0;
    const qM = parseInt(qtyMuslimahEl2.value,10) || 0;
    const sum = qL + qM;
    const tier = tierForTotal(sum);
    const isEarlybird = state.selectedCombo && state.selectedCombo.type === 'earlybird';
    const rates = ratesForComboState(sum, state.selectedCombo);
    const tierLabel = isEarlybird ? 'Early Bird' : (tier ? `${tier} helai` : null);
    rateLakiLabelEl.textContent = tierLabel ? `Harga combo ${tierLabel}: ${fmt(rates.laki)}/pc` : 'Pilih combo dahulu';
    rateMuslimahLabelEl.textContent = tierLabel ? `Harga combo ${tierLabel}: ${fmt(rates.muslimah)}/pc` : 'Pilih combo dahulu';

    if(!state.selectedCombo){
      splitStatusEl.className = 'split-status';
      splitStatusEl.textContent = 'Belum pilih combo.';
    }else if(state.selectedCombo.type === 'fixed' || state.selectedCombo.type === 'earlybird'){
      splitStatusEl.className = 'split-status ' + (sum === state.selectedCombo.target ? 'match' : 'mismatch');
      splitStatusEl.textContent = sum === state.selectedCombo.target
        ? `Sempurna — ${sum} / ${state.selectedCombo.target} helai dibahagikan.`
        : `Dipilih ${sum} / ${state.selectedCombo.target} helai. Sila laraskan supaya jumlah tepat ${state.selectedCombo.target}.`;
    }else{
      splitStatusEl.className = 'split-status ' + (sum > 0 ? 'match' : '');
      splitStatusEl.textContent = sum > 0 ? `Jumlah keseluruhan: ${sum} helai (tier harga ${tier} helai).` : 'Masukkan kuantiti Laki / Muslimah.';
    }

    if(state.selectedCombo){
      blockEl.querySelectorAll('.bk-stepper button[data-step="1"]').forEach(btn=>{
        const t = document.getElementById(btn.dataset.target);
        if(t === qtyLakiEl2 || t === qtyMuslimahEl2){
          btn.disabled = comboRemainingThis(t) <= 0;
        }
      });
    }

    const sizeSumL = getSizeSum(p+'szL');
    if(qL > 0){
      sizeStatusLakiEl.className = 'size-status ' + (sizeSumL === qL ? 'match' : 'mismatch');
      sizeStatusLakiEl.textContent = `Saiz: ${sizeSumL} / ${qL} helai`;
    }else{
      sizeStatusLakiEl.className = 'size-status';
      sizeStatusLakiEl.textContent = '';
    }
    const sizeSumM = getSizeSum(p+'szM');
    if(qM > 0){
      sizeStatusMuslimahEl.className = 'size-status ' + (sizeSumM === qM ? 'match' : 'mismatch');
      sizeStatusMuslimahEl.textContent = `Saiz: ${sizeSumM} / ${qM} helai`;
    }else{
      sizeStatusMuslimahEl.className = 'size-status';
      sizeStatusMuslimahEl.textContent = '';
    }
    [p+'szL', p+'szM'].forEach(prefix=>{
      SIZES.forEach(sz=>{
        const el = document.getElementById(prefix + '_' + sz);
        if(el) el.max = sizeRemaining(prefix, sz);
      });
    });
    [[p+'szL',p+'lycL'],[p+'szM',p+'lycM']].forEach(([sizePrefix,lycPrefix])=>{
      SIZES.forEach(sz=>{
        const el = document.getElementById(lycPrefix + '_' + sz);
        if(el) el.max = lycraMaxForSize(sizePrefix, sz);
      });
    ...
    const lycSumL = getSizeSum(p+'lycL');
    lycraStatusLakiEl.textContent = lycSumL > 0 ? `Lycra: ${lycSumL} / ${qL} helai dinaik taraf` : '';
    const lycSumM = getSizeSum(p+'lycM');
    lycraStatusMuslimahEl.textContent = lycSumM > 0 ? `Lycra: ${lycSumM} / ${qM} helai dinaik taraf` : '';

    receiptBodyEl.innerHTML = buildReceiptHtml(computeThis());
  }
  state.compute = computeThis;
  state.update = updateThis;

  buildSizeGrid(p+'SizeGridLaki', p+'szL', updateThis);
  buildSizeGrid(p+'SizeGridMuslimah', p+'szM', updateThis);
  buildLycraGrid(p+'LycraGridLaki', p+'lycL', p+'szL', updateThis);
  buildLycraGrid(p+'LycraGridMuslimah', p+'lycM', p+'szM', updateThis);

  function enableQtyControls(enabled){
    blockEl.querySelectorAll('.bk-stepper button, .bk-stepper input').forEach(el=>{ el.disabled = !enabled; });
    setSizeInputsEnabled(p+'szL', enabled);
    setSizeInputsEnabled(p+'szM', enabled);
    setSizeInputsEnabled(p+'lycL', enabled);
    setSizeInputsEnabled(p+'lycM', enabled);
  }

  blockEl.querySelectorAll('#'+p+'ComboCards .combo-card').forEach(card=>{
    card.addEventListener('click', ()=>{
      blockEl.querySelectorAll('#'+p+'ComboCards .combo-card').forEach(c=>c.classList.remove('selected'));
      card.classList.add('selected');
      const type = card.dataset.type;
      const target = parseInt(card.dataset.target,10);
      state.selectedCombo = { type, target: (type === 'fixed' || type === 'earlybird') ? target : null };
      enableQtyControls(true);
      if(type === 'earlybird'){
        comboHintEl.textContent = 'Combo Early Bird dipilih — 1 helai sahaja.';
      }else if(type === 'fixed'){
        comboHintEl.textContent = `Combo ${target} helai dipilih. Bahagikan ${target} helai antara Laki dan Muslimah di bawah.`;
      }else{
        comboHintEl.textContent = 'Kuantiti sendiri dipilih. Harga akan ikut jumlah keseluruhan yang anda masukkan.';
      }
      comboHintEl.classList.add('ok');
      updateThis();
    });
  });

  blockEl.querySelectorAll('#'+p+'DeliveryCards .delivery-card').forEach(card=>{
    card.addEventListener('click', ()=>{
      blockEl.querySelectorAll('#'+p+'DeliveryCards .delivery-card').forEach(c=>c.classList.remove('selected'));
      card.classList.add('selected');
      state.deliveryMethod = card.dataset.method;
      deliveryHintEl.textContent = state.deliveryMethod === 'home'
        ? 'Hantar ke rumah dipilih — caj pos +RM8 (1–4 helai) atau +RM15 (5 helai ke atas). Pastikan alamat penghantaran diisi.'
        : 'Hantar ke sekolah dipilih — percuma.';
      deliveryHintEl.classList.add('ok');
      addrFieldEl.style.display = state.deliveryMethod === 'home' ? 'block' : 'none';
      if(state.deliveryMethod !== 'home') document.getElementById(p+'CustAddr').value = '';
      updateThis();
    });
  });

  blockEl.querySelectorAll('.bk-stepper button').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      if(btn.disabled) return;
      const target = document.getElementById(btn.dataset.target);
      const step = parseInt(btn.dataset.step,10);
      let val = Math.max(0, (parseInt(target.value,10)||0) + step);
      if((target === qtyLakiEl2 || target === qtyMuslimahEl2) && step > 0){
        val = Math.min(val, comboRemainingThis(target));
      }
      target.value = val;
      updateThis();
    });
  });
  [qtyLakiEl2, qtyMuslimahEl2].forEach(el=>{
    el.addEventListener('input', ()=>{
      if(el.value === '') return;
      let val = Math.max(0, parseInt(el.value,10) || 0);
      val = Math.min(val, comboRemainingThis(el));
      el.value = val;
      updateThis();
    });
  });

  blockEl.querySelector('.bk-remove-btn').addEventListener('click', ()=>{
    delete bkBlocks[idx];
    delete QTY_EL_BY_PREFIX[p+'szL'];
    delete QTY_EL_BY_PREFIX[p+'szM'];
    delete LYC_PREFIX_BY_SIZE_PREFIX[p+'szL'];
    delete LYC_PREFIX_BY_SIZE_PREFIX[p+'szM'];
    blockEl.remove();
    if(Object.keys(bkBlocks).length === 0) bkAddOrder();
  });
...
function bkAddOrder(){
  const idx = bkOrderCounter++;
  document.getElementById('bkOrdersContainer').insertAdjacentHTML('beforeend', bkOrderBlockHtml(idx));
  bkWireOrderBlock(idx);
}
document.getElementById('bkAddOrderBtn').addEventListener('click', ()=> bkAddOrder());
bkAddOrder();

document.getElementById('bkSubmitAllBtn').addEventListener('click', async ()=>{
  const bkGlobalErr = document.getElementById('bkGlobalErr');
  const submitAllBtn = document.getElementById('bkSubmitAllBtn');
  bkGlobalErr.style.display = 'none';
  const indices = Object.keys(bkBlocks).map(Number).sort((a,b)=>a-b);
  if(indices.length === 0){
    bkGlobalErr.textContent = 'Tiada order untuk dihantar. Klik "+ Tambah Order Lain" dahulu.';
    bkGlobalErr.style.display = 'block';
    return;
  }
  submitAllBtn.disabled = true;
  submitAllBtn.textContent = 'Menghantar...';
  let anySent = false, anyError = false;

  for(const idx of indices){
    const state = bkBlocks[idx];
    if(!state) continue;
    const p = 'bk' + idx;
    const formErrEl = document.getElementById(p+'FormErr');
    const statusEl = document.getElementById(p+'SubmitStatus');
    formErrEl.style.display = 'none';

    const agentName = document.getElementById(p+'AgentName').value.trim();
    const schoolName = document.getElementById(p+'SchoolName').value;
    const custName = document.getElementById(p+'CustName').value.trim();
    const custPhone = document.getElementById(p+'CustPhone').value.trim();
    const custAddr = document.getElementById(p+'CustAddr').value.trim();
    const notes = document.getElementById(p+'OrderNotes').value.trim();
    const o = state.compute();
    ...
    if(!agentName){ fail('Sila masukkan nama ejen.'); continue; }
    if(!schoolName){ fail('Sila pilih sekolah.'); continue; }
    if(!custName){ fail('Sila masukkan nama pelanggan.'); continue; }
    if(!state.selectedCombo){ fail('Sila pilih combo dahulu.'); continue; }
    if(o.qL === 0 && o.qM === 0){ fail('Sila masukkan sekurang-kurangnya 1 kuantiti baju.'); continue; }
    if((state.selectedCombo.type === 'fixed' || state.selectedCombo.type === 'earlybird') && o.qtySum !== state.selectedCombo.target){
      fail(`Jumlah mesti tepat ${state.selectedCombo.target} helai untuk combo ini (sekarang ${o.qtySum}).`);
      continue;
    }
    const sizeLaki = getSizeCounts(p+'szL');
    const sizeMuslimah = getSizeCounts(p+'szM');
    if(o.qL > 0 && getSizeSum(p+'szL') !== o.qL){ fail(`Sila bahagikan saiz Baju Laki supaya jumlah tepat ${o.qL} helai.`); continue; }
    if(o.qM > 0 && getSizeSum(p+'szM') !== o.qM){ fail(`Sila bahagikan saiz Baju Muslimah supaya jumlah tepat ${o.qM} helai.`); continue; }
    if(!state.deliveryMethod){ fail('Sila pilih kaedah penghantaran.'); continue; }
    if(state.deliveryMethod === 'home' && !custAddr){ fail('Sila isi alamat penghantaran untuk hantar ke rumah.'); continue; }

    const order = {
      agentName, schoolName, custName, custPhone, custAddr,
      notes: (notes ? notes + ' | ' : '') + '(Bulk/Offline entry oleh admin)',
      comboType: state.selectedCombo.type, comboTarget: state.selectedCombo.target,
      qtyLaki: o.qL, qtyMuslimah: o.qM,
      upgradeQtyLaki: o.upgL, upgradeQtyMuslimah: o.upgM, lycraSurcharge: o.lycraSurcharge,
      sizeSurchargeLaki: o.surL.total, sizeSurchargeMuslimah: o.surM.total,
      deliveryMethod: o.deliveryMethod, deliveryFee: o.deliveryFee,
      lycraSizeLaki: getSizeCounts(p+'lycL'), lycraSizeMuslimah: getSizeCounts(p+'lycM'),
      sizeLaki, sizeMuslimah,
      priceLaki: o.rL, priceMuslimah: o.rM,
      subtotalLaki: o.subL, subtotalMuslimah: o.subM,
      total: o.total, commission: o.commission,
    ...
    statusEl.className = 'split-status';
    statusEl.textContent = 'Menghantar...';
    try{
      const res = await fetch(SHEETS_WEBAPP_URL, { method: 'POST', body: JSON.stringify(order) });
      const result = await res.json();
      if(!result.ok) throw new Error(result.error || 'Gagal simpan ke Google Sheet');
      statusEl.className = 'split-status match';
      statusEl.textContent = `✓ Dihantar — ${custName}`;
      anySent = true;
      delete bkBlocks[idx];
      delete QTY_EL_BY_PREFIX[p+'szL'];
      delete QTY_EL_BY_PREFIX[p+'szM'];
      delete LYC_PREFIX_BY_SIZE_PREFIX[p+'szL'];
      delete LYC_PREFIX_BY_SIZE_PREFIX[p+'szM'];
      document.getElementById(p+'Block').remove();
    }catch(e){
      statusEl.className = 'split-status mismatch';
      statusEl.textContent = 'Ralat — gagal hantar, cuba lagi.';
      anyError = true;
    }
  }

  submitAllBtn.disabled = false;
  submitAllBtn.textContent = 'Hantar Semua Pesanan';
  if(anySent){
    showToast('Order offline dihantar.');
    loadOrders();
  }
  if(Object.keys(bkBlocks).length === 0) bkAddOrder();
  if(anyError){
    bkGlobalErr.textContent = 'Sebahagian order gagal/tidak lengkap — semak mesej ralat pada setiap order di atas.';
    bkGlobalErr.style.display = 'block';
  }
});
...
const ADMIN_PASSWORD = 'hendrixx26';
if(tabAdminBtn){
...
let ordersPage = 1;
let ordersCurrentList = [];

function renderOrders(orders){
  ordersCurrentList = orders;
  const tbody = document.getElementById('ordersTbody');
  if(orders.length === 0){
...
function renderOrdersPagination(totalItems){
  ...
  const goTo = (p)=>{ ordersPage = p; renderOrders(ordersCurrentList); };
  const windowSize = 5;
  let startPg = Math.max(1, ordersPage - Math.floor(windowSize / 2));
  let endPg = Math.min(totalPages, startPg + windowSize - 1);
  startPg = Math.max(1, endPg - windowSize + 1);

  let html = `<button type="button" data-pg="prev" ${ordersPage <= 1 ? 'disabled' : ''}>‹ Prev</button>`;
  if(startPg > 1){
    html += `<button type="button" data-pg="1">1</button>`;
    if(startPg > 2) html += `<span class="pg-info">…</span>`;
  }
  for(let p = startPg; p <= endPg; p++){
    html += `<button type="button" class="${p === ordersPage ? 'active' : ''}" data-pg="${p}">${p}</button>`;
  }
  if(endPg < totalPages){
    if(endPg < totalPages - 1) html += `<span class="pg-info">…</span>`;
    html += `<button type="button" data-pg="${totalPages}">${totalPages}</button>`;
  }
  html += `<button type="button" data-pg="next" ${ordersPage >= totalPages ? 'disabled' : ''}>Next ›</button>`;
  html += `<span class="pg-info">Jumlah: ${totalItems} order</span>`;
  container.innerHTML = html;

  container.querySelectorAll('button[data-pg]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      const v = btn.dataset.pg;
      if(v === 'prev') goTo(Math.max(1, ordersPage - 1));
      else if(v === 'next') goTo(Math.min(totalPages, ordersPage + 1));
      else goTo(parseInt(v, 10));
    });
  });
}
